...
var K = function() {
    ...
    Class.extend = function(className, prop) {
        ...
        for (var name in prop) {
            ...
            // Check for special stuff like statics, ... and don't do anything
            switch (name) {
                case 'requires':
                case 'statics':
                    continue;
            ...
            if (_isFunction(prop[name]) && _isFunction(base[name]) && fnTest.test(prop[name])) {
                ...
                if (!_isFunction(prop[n])) {
                    var accessorName = 'get' + uName;
                    // Check if the accessor already exist and throw an error if it does
                    if (!_isEmpty(prototype[accessorName]) && !nonAccessorProps.indexOf(accessorName))
        ...
        if (!_isEmpty(abstracts) && _isArray(abstracts)) {
            ...
            for (var i=0; i<abstracts.length; i++) {
                ...
                if (_isEmpty(prototype[abstractName])) {
        ...
        core: {
            ...
            // The callback is called every time an object has been found
                ...
                fromString: function(html) {
                    var fragDiv = K.core.Dom.create('div');
                    fragDiv.innerHTML = html;
                    return fragDiv.firstChild;
                },
